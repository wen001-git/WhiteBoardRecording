...
- v3 白板文档的 `teleprompter{text,speed,fontSize,color}` 保存整份讲稿、滚动速度、字号和统一字色；导入或恢复时必须停止播放、回到编辑态并把滚动位置归零，显隐、窗口位置、播放和滚动进度不得持久化。缺字段的旧文件使用空讲稿和默认设置，数值需限制在现有滑块范围内，非法或缺失颜色回退为白色。
...
| 日期 | 变更内容 |
|------|----------|
| 2026-07-21 | 为提词器增加可保存的统一文字颜色并同步编辑态和播放态；why：让用户按背景和阅读偏好调整讲稿字色 |
| 2026-07-21 | 为提词器增加视口内受限的右下角拖拽缩放；why：让长讲稿获得更大的可浏览面积且避免窗口越界 |
| 2026-07-20 | 将提词器讲稿、速度和字号纳入 v3 白板文档并保留旧本机讲稿迁移；why：让文件导入和浏览器草稿都能可靠恢复讲稿而不保存临时播放状态 |
