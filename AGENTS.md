...
- 两个白板版本的新画笔已改用 Excalicord 同款 Perfect Freehand 核心：三档为细/粗/特粗（1/2/4，默认粗），固定使用 4.25 size 系数、0.6 thinning、0.5 smoothing/streamline 和正弦压力缓动；鼠标按点距渐进模拟压力，Apple Pencil 保存真实压力，首尾与急转圆角由一次闭合轮廓生成。v9 数据参与选择、擦除、缩放、复制和录制，旧固定笔画及实验性 v7 `f` 笔画保持可打开。
...
- 两个白板版本的提词器讲稿、速度和字号已纳入 v9 白板文档，可随 `.json` 文件和浏览器草稿保存、恢复；打开后回到未播放的编辑态，普通版与 Pro 版仍共用兜底缓存。
- 两个白板版本的提词器支持先选中文字再单独着色，一份讲稿可使用多种字色；局部颜色在编辑态与播放态同步，并随 v9 白板文档及浏览器草稿保存、恢复。
...
- 两个白板版本均可从左上角设置整份白板底色，并允许当前幻灯片单独覆盖或恢复继承；颜色写入 v9 文档、撤销历史、鸟瞰图、笔迹播放和白板录像。
...
- 两个白板版本均支持为每张幻灯片设置无、淡化、推入或擦除进入转场、三档速度，以及无声音、翻书、轻柔滑动或柔和提示音与独立音量；编辑和正式录制切页会同步播放，声音混入成品音轨，全部设置写入 v9 文档并可一键应用到全部幻灯片。
- 两个白板版本的幻灯片动画菜单可把当前单选或多选对象加入逐页“元素操作栏”；只有绑定对象受显示控制，用户可任意点击命名按钮触发约 200ms canvas 淡入、单独显隐或全部显隐。“笔迹”按钮保持紧贴幻灯片上方的原位置，元素操作栏固定在它正上方，项目过多时自动换行向上展开并全部显示，不使用横向滚动；操作栏不入镜，暂停录制时的准备动作不写入视频帧。绑定和稳定对象 ID 写入 v9 文档及撤销历史，当前显隐状态只保留在本次打开期间。
...
| 2026-07-27 | 修复公共入口双语脚本嵌套与缺失资源引用，并新增发布前脚本语法和资源存在性检查；why：避免 JavaScript 在会话检查前停止执行，导致首页永久停留在登录检查状态 |
