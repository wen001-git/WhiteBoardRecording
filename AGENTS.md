...
- 两个白板版本的顶部控件会按真实尺寸自动避让：电脑与 iPad 都把新建、打开和保存统一收进“文件”菜单，空间不足时再把形状、素材和清空收进紧凑绘画栏；极窄视口使用单行滚动兜底，iPad 触控按钮保持至少 44px。
...
| 日期 | 变更内容 |
|------|----------|
| 2026-07-23 | 顶部文件、绘画与录制控件改为按真实尺寸避让，电脑与 iPad 统一使用文件菜单，并为窄屏增加紧凑工具组和单行滚动兜底；why：防止左侧按钮与居中绘画栏重叠，同时保持跨设备一致 |
| 2026-07-22 | 统一页面头像、设置预览和两类录制的纹理保留美颜、暗沉淡化和红润肤色，并以保高光亮度曲线替代白色蒙层；why：改善黑眼圈与脸色，同时避免五官变糊 |
| 2026-07-22 | 新增和插入幻灯片改为继承当前相邻页面的实际宽高；why：修复已有页面切换比例后，新页面仍使用预设标准尺寸而导致同一比例大小不一致 |
