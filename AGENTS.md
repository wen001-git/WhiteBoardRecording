...
- 已登录 Pro 用户的右下角常显“推荐给朋友”和账号菜单；联系/推荐弹窗可复制动态 `paywall.json` 微信号、扫描现有二维码、编辑并复制推荐语，支持可用时的系统分享，首次点击不会自动改写剪贴板。
...
| 2026-07-18 | 已登录账号区新增常显推荐入口、联系/推荐共用弹窗、动态微信号复制和显式系统分享；why：让用户无需退出即可找到作者并把产品推荐给朋友 |
