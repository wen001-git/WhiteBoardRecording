...
- Neon 成功登录会在响应返回后异步记录 IP、设备、浏览器和时间；管理页展示每个账号最近 100 条，并提示任意 1 小时窗口内的多 IP 情况，本地静态账号不记录。
...
| 日期 | 变更内容 |
|------|----------|
| 2026-07-18 | 增加 Neon 成功登录 IP 历史与一小时多 IP 提醒，并将审计写入移出登录响应关键路径；why：辅助识别账号共享且不拖慢登录 |
| 2026-07-17 | 增加整份白板底色与当前幻灯片覆盖，并同步保存、撤销、鸟瞰图、笔迹播放和录像；why：让课件编辑和录制成品使用一致的视觉底色 |
| 2026-07-17 | 为两个白板版本的录制设置增加可选文字水印并写入白板录制/录屏成品；why：让用户导出视频时可标注讲师名、账号或课程品牌 |
