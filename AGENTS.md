...
| 日期 | 变更内容 |
|------|----------|
| 2026-07-18 | 公共登录页新增动态微信号复制、成功反馈及按需展开二维码；why：方便未购用户直接在微信搜索或扫码添加，同时不干扰已购用户登录 |
| 2026-07-18 | 公共入口改为先恢复静态/Neon 会话并自动进入白板，失败或 5 秒超时才显示登录页；why：避免已登录用户重复看到登录表单 |
| 2026-07-18 | 将测试策略改为按修改风险分级，仅复杂或关键路径修改运行相关最小测试；why：减少低风险修改后的重复验证开销 |
