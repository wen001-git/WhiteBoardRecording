...
- **笔迹按钮避让录制框(2026-07-12)**：旧的 `.slideFrame` 内部「笔迹」按钮被绿色 `#recFrame`（z-index 56）遮挡，已改为全局 DOM 浮层 `#slideRevealFloatBtn`（z-index 58），由 `positionSlideRevealButton()` 跟随 active slide 定位；优先放在幻灯片右上外侧，右侧不足时放到顶部/左侧/底部外侧，窄屏自动压缩为仅图标。按钮仍只调用 `startSlideReveal(state.activeSlide)`，不写入 canvas，不进入录制导出，也不改变幻灯片或录制框坐标。
...
| 日期 | 变更内容 |
|------|---------|
| 2026-07-12 | 修复录制取景框遮挡笔迹按钮：将按钮从幻灯片框内部改为高层级全局浮层，并按 active slide 外侧自适应定位；why：进入幻灯片录制模式时无需挪动幻灯片也能点击笔迹动画，且避免用户误以为按钮会进入视频 |
