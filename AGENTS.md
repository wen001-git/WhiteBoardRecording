...
- 两个白板版本的提词器讲稿、速度和字号已纳入 v4 白板文档，可随 `.json` 文件和浏览器草稿保存、恢复；打开后回到未播放的编辑态，旧本机讲稿可迁移且普通版与 Pro 版仍共用兜底缓存。
- 两个白板版本的提词器支持先选中文字再单独着色，一份讲稿可使用多种字色；局部颜色在编辑态与播放态同步，并随 v4 白板文档及浏览器草稿保存、恢复。
...
- 两个白板版本均可从左上角设置整份白板底色，并允许当前幻灯片单独覆盖或恢复继承；颜色写入 v4 文档、撤销历史、鸟瞰图、笔迹播放和白板录像。
...
- 两个白板版本均支持为每张幻灯片设置无、淡化、推入或擦除进入转场及三档速度；缩略图和左右键切页会在编辑与录制中自动播放，设置写入 v4 文档并可一键应用到全部幻灯片。
...
| 2026-07-21 | 为普通版与 Pro 版增加幻灯片文字框逐行淡入动画；why：支持讲解要点按版面顺序出现并进入录制 |
