...
- 两个白板版本的提词器讲稿、速度和字号已纳入 v3 白板文档，可随 `.json` 文件和浏览器草稿保存、恢复；打开后回到未播放的编辑态，旧本机讲稿可迁移且普通版与 Pro 版仍共用兜底缓存。
- 两个白板版本的提词器支持先选中文字再单独着色，一份讲稿可使用多种字色；局部颜色在编辑态与播放态同步，并随 v3 白板文档及浏览器草稿保存、恢复。
...
| 2026-07-21 | 为普通版与 Pro 版提词器增加可持久化的选区文字颜色，并绕开颜色面板造成的焦点丢失；why：让选中文字能可靠着色且一份讲稿可使用多种字色 |
