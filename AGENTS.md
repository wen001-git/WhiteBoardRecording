...
- [ ] 真机验证 Chrome/Safari 的白板激光笔、录屏光标隐藏降级、整屏裁剪、摄像头/麦克风/系统声与 MP4 成品。
...
| 2026-07-29 | 为逐页元素操作栏增加统一动作音效、音量和录制混流，并升级文档为 v10；why：让元素淡入在视频中获得同步听觉反馈，同时避免重复点击或批量显示产生叠音 |
| 2026-07-28 | 新增逐页元素操作栏、稳定对象 ID 和录制友好的 200ms 淡入，并将可换行操作栏固定在原“笔迹”按钮上方；why：支持视频创作者按任意顺序直接控制元素出现，同时让全部按钮直接可见且不遮挡页面 |
