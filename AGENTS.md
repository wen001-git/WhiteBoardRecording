...
npm install
# 优先跑与本次改动直接相关的测试；只有跨模块/发布前/用户明确要求时才跑全量 npm test
...
- **测试要求**：每个阶段性修改后优先做覆盖本次核心路径的最小自动化或可执行验证；不要为小范围改动默认跑全量 `npm test` 或强制浏览器预览，只有跨模块风险、发布前检查或用户明确要求时才做全量验证；失败则修复后重测。
...
| 2026-07-15 | 新增静态 Pro 账号临时方案：`accounts.json` 保存 hash 账号，`account-admin1.html` 管理/生成静态账号，`index.html`/`whiteboard-pro.html` 支持静态登录；why：用户需要立刻发放不限制设备的 Pro 登录账号，且不等待 Neon 后端账号任务完成 |
