...
- 两个白板版本的顶部控件会按真实尺寸自动避让：左上角只保留单色线框“主菜单”和高频“保存备份”，竖向主菜单收纳新建、打开、保存及画布背景；空间不足时再把形状、素材和清空收进紧凑绘画栏，极窄视口使用单行滚动兜底，iPad 触控按钮保持至少 44px。
...
- 两个白板版本的提词器讲稿、速度和字号已纳入 v6 白板文档，可随 `.json` 文件和浏览器草稿保存、恢复；打开后回到未播放的编辑态，普通版与 Pro 版仍共用兜底缓存。
- 两个白板版本的提词器支持先选中文字再单独着色，一份讲稿可使用多种字色；局部颜色在编辑态与播放态同步，并随 v6 白板文档及浏览器草稿保存、恢复。
...
- 两个白板版本均可从左上角设置整份白板底色，并允许当前幻灯片单独覆盖或恢复继承；颜色写入 v6 文档、撤销历史、鸟瞰图、笔迹播放和白板录像。
...
- 两个白板版本均支持为每张幻灯片设置无、淡化、推入或擦除进入转场、三档速度，以及无声音、翻书、轻柔滑动或柔和提示音与独立音量；编辑和正式录制切页会同步播放，声音混入成品音轨，全部设置写入 v6 文档并可一键应用到全部幻灯片。
...
| 日期 | 变更内容 |
|------|----------|
| 2026-07-24 | 画布背景改为带当前底色色点的彩色调色盘直达按钮，保存入口统一恢复软盘图标；why：减少设置步骤、提升入口辨识度并强化保存语义 |
| 2026-07-23 | 摄像头与麦克风改为紧凑双分段线框控制，设置页移除重复摄像头开关并增加麦克风设备记忆和实时音量条；why：让普通用户用一套状态直观组合仅画面、仅声音或同时录制 |
| 2026-07-23 | 文档恢复和切页时由当前幻灯片真实宽高同步右侧尺寸与录制比例；why：修复刷新后实际 3:4 页面与两个 16:9 控件不一致 |
