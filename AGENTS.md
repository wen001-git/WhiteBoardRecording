...
- 两个白板版本的单选和多选对象均支持 `Ctrl/Cmd+C` 后用 `Ctrl/Cmd+V` 粘贴，也可用 `Ctrl/Cmd+D` 直接复制并粘贴；副本保留样式、相对位置和图层顺序并可一次撤销。
...
| 日期 | 变更内容 |
|------|----------|
| 2026-07-21 | 为普通版与 Pro 版增加单选和多选对象的 Ctrl/Cmd+C、V、D 复制粘贴；why：让白板元素可通过标准快捷键快速复用 |
