...
- **彩铅小人贴纸(2026-07-11)**：顶部工具栏图片按钮旁新增 `#stickerBtn`，弹出 `#stickerPopover` 的 8 个原创透明 PNG 贴纸（无所不能、开心、疑惑、焦虑、灵感、低落、专注、庆祝）；素材来自第一版生成的彩铅小人集合图，按 4×2 网格裁切并用边缘泛洪去白底，只保留透明人物贴纸，通过 `STICKERS` 内联为 `data:image/png;base64`，不新增外部文件，保持单 HTML。点击贴纸后调用 `beginPendingImage(sticker.src,sticker.w||240,sticker.h||240)`，复用现有图片对象流程，因此可在画布单击放置、拖动、缩放、删除、撤销/重做，并进入录制导出。
